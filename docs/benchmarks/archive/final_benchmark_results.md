...
- Subsequent requests: 13-14ms prompt processing (**KV cache effect**)
- Token generation: **75-80 tokens/second** consistently
- Time per output token: **~13ms (TPOT)**
- Total latency range: 380-885ms depending on cache state
...
**Analysis:**
- All requests hit llama.cpp's KV cache (prompts were short and similar)
- Gateway correctly routes to backend
- **Minimal gateway overhead** - matches direct cache-hit performance
...
**Analysis:**
- This is the most reliable throughput measurement
- Embeddings don't have KV cache interference
- Gateway handles high request volume effectively
- Sub-10ms latency demonstrates low overhead

---

## 4. Gateway Metrics

Prometheus metrics after all tests:
...
- p95 queue wait < 5ms (admission control working)
...
- Concurrency tests show artificially high throughput (45+ req/s) due to cache
- Real production throughput is **generation-bound**: ~75 tokens/sec ÷ 40 tokens/request = **~2 req/s** for completions
...
✅ "Implemented token-aware admission using weighted semaphore, so requests consume budget proportional to estimated token cost"  
✅ "Built multi-backend routing with weighted selection based on (load/capacity) + (p95_latency × 10) and circuit breakers"  
✅ "Propagated context cancellation end-to-end so client disconnects free slots immediately"  
✅ "Instrumented with 23 Prometheus metrics using histograms, separating queue wait from generation time"  
...
> 
> The core validation is that admission control prevents overload, token-aware scheduling is operational, metrics show healthy queue behavior, and the gateway overhead is minimal. The infrastructure works correctly; measuring it just requires understanding what the backend is actually doing."
...
- **Testing:** Functional testing passed, would need load testing with token variance to measure benefit
...
## 10. What's Next (If Continuing)

### To Get Real Completion Throughput
1. Generate corpus of 1000+ truly unique prompts
2. Run 10-minute sustained load test
3. Measure over time window (not per-request)
4. Result will still be generation-bound (~2 req/s) but more representative

### To Test Multi-Backend
1. Start 3 llama.cpp instances on ports 8080, 8081, 8082
2. Configure gateway with all three backends
3. Run load test, verify distribution
4. Kill one backend, measure failover and circuit breaker

### To Measure Batching Benefit
1. Test embeddings at batch sizes 1, 4, 8, 16, 32
2. Measure throughput improvement curve
3. This would be a strong result (built the batcher yourself)
...
**Next step:** Update INTERVIEW_GUIDE.md with these findings and remove all invalid claims from documentation.
